...
SAVE_PATH = '/ssd/peterwg/ShapeNetSO3_train'#'/home/peterwg/dataset/ShapeNetRendering3d'
TEST_PATH = '/home/peterwg/dataset/ShapeNetSO3_test'
NUM_SPLITS = 50
SUBS = 30
...
def R_traj(R):
    indexs = np.random.choice(NUM_SPLITS, size=SUBS, replace=False)
    R = R[indexs]
    axs = np.random.choice(3, 30)
    R_out = []
    for i_R, ax in zip(R, axs):
        if ax == 0:
            rots = torch.Tensor([Rotation.from_euler('xyz', [[ -1*angle_int,0,0]], degrees=True).as_dcm(), Rotation.from_euler('xyz', [[ 0,0,0]], degrees=True).as_dcm(), Rotation.from_euler('xyz', [[ 1*angle_int,0,0]], degrees=True).as_dcm()] ).squeeze()
        elif ax == 1:
            rots = torch.Tensor([Rotation.from_euler('yxz', [[ -1*angle_int,0,0]], degrees=True).as_dcm(), Rotation.from_euler('yxz', [[ 0,0,0]], degrees=True).as_dcm(), Rotation.from_euler('yxz', [[ 1*angle_int,0,0]], degrees=True).as_dcm()] ).squeeze()
        else:
            rots = torch.Tensor([Rotation.from_euler('zxy', [[ -1*angle_int,0,0]], degrees=True).as_dcm(), Rotation.from_euler('zxy', [[ 0,0,0]], degrees=True).as_dcm(), Rotation.from_euler('zxy', [[ 1*angle_int,0,0]], degrees=True).as_dcm()] ).squeeze()
        R_out.append( rots@i_R.unsqueeze(0) )
        #if ax ==0:
    R_out = torch.cat(R_out)
    return R_out, indexs, axs
...
for index in range(len(dataset_class)):
    ...
    if os.path.isdir( os.path.join(SAVE_PATH, model["synset_id"], model["model_id"]) ):
        continue
    '''if not os.path.isdir( os.path.join(TEST_PATH, model["synset_id"], model["model_id"]) ):
        continue'''
    ...
    R, indexs, axs = R_traj(R)
    T = torch.cat([T]*SUBS*3)
    ...
    for i, (img, index, ax) in enumerate( zip(images_by_idxs.cpu().numpy(), np.repeat(indexs, 3), np.repeat(axs, 3))):
        imageio.imwrite( os.path.join(SAVE_PATH, model["synset_id"], model["model_id"], str(index)+"_"+str(ax)+"_"+str(i%3)+".png"), rgba2rgb(img))
